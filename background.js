// Load config FIRST so it's available in the service worker context
// NOTE: This file must set globalThis.EVENTSNAP_CONFIG = { ... }
// IMPORTANT: In MV3 SW, importScripts will FAIL if the file isn't actually present in the loaded extension.
// We guard this so the service worker can still start.
try {
  importScripts("popup.config.local.js");
} catch (e) {
  // Optional: uncomment to see this in the SW console
  // console.warn("[BG] Could not load popus.config.local.js via importScripts:", e);
}

// Supabase library
importScripts("vendor/supabase.js");

// Keep your key in your local copy if you insist.
// (Not including your real key here.)
const OPENAI_API_KEY = "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";
...
// Fallback config if popus.config.local.js isn't present in the built/loaded extension.
// You can remove this once your config file is reliably copied into the extension output.
const EVENTSNAP_CONFIG_FALLBACK = Object.freeze({
...
function getEffectiveConfig(message) {
  // Priority:
  // 1) message.config (popup-provided)
  // 2) globalThis.EVENTSNAP_CONFIG (loaded from popus.config.local.js if present)
  // 3) fallback hardcoded config (so SW doesn't break)
  const fromMessage = message?.config && typeof message.config === "object" ? message.config : null;
  const fromGlobal = globalThis.EVENTSNAP_CONFIG && typeof globalThis.EVENTSNAP_CONFIG === "object" ? globalThis.EVENTSNAP_CONFIG : null;
  return fromMessage || fromGlobal || EVENTSNAP_CONFIG_FALLBACK;
}
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  // const cfg = getEffectiveConfig(message);
  // console.log("[BG] message", message.type, "cfg:", cfg);
  ...
  if (message.type === "AUTH_SIGN_IN") {
    (async () => {
      try {
        const config = getEffectiveConfig(message);
...
function assertSupabaseConfig(config) {
  ...
  if (
    ...
    throw new Error(
      "Missing Supabase config. Provide supabaseUrl and supabaseAnonKey (via popup message config or EVENTSNAP_CONFIG)."
    );
  }
}
  ...
  return new Promise((resolve, reject) => {
    ...
    try {
      chrome.identity.launchWebAuthFlow({ url, interactive }, (callbackUrl) => {
        ...
        console.log("[OAuth] lastError:", chrome.runtime.lastError);
        if (runtimeError) {
